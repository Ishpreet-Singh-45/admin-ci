...
class AuthController extends Controller
{
	...
	public function __construct()
	{
		...
		$this -> loginModel = model('GoogleAuth');
	}
	...
	public function login()
	{
		 ...
		 * 
		 * Library at: App/Libraries/vendor
		 */

		require APPPATH . 'Libraries/vendor/autoload.php';

		$google_client = new \Google_Client();
		$google_client -> setAuthConfig(APPPATH . 'Libraries/google-auth_credentials.json');
		...
		if($this -> request -> getVar('code'))
		{
			...
			if(! isset($token['error']))
			{
				...
				$gdata = $google_service -> userinfo -> get(); // get profile data


				// save the login info in database
				if($this -> loginModel -> userExist($gdata['id']))
				{
					// update if returning user
					$userData = [
						'Email' => $gdata['email'],
						'First_Name' => $gdata['givenName'],
						'Last_Name' => $gdata['familyName'],
						'P_Pic' => $gdata['picture']
					];

					$this -> loginModel -> updateUserData($userData, $gdata['id']);
				}else
				{
					// insert if new user
					$userData = [
					...
					$this -> loginModel -> setNewUser($userData);
		...
		// create auth url if token not found
		...
		return $this->_render($this->config->views['login'], ['config' => $this->config, 'loginButton' => $data['loginButton'] ?? ""]);
	}
	...
	{
		if ($this->auth->check() || $this -> session -> has('user'))
		{
